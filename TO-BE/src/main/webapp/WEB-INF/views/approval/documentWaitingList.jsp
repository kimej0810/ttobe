...
<% 
	List<ApprovalVO> waiting = (List<ApprovalVO>)request.getAttribute("waiting");
...
<html>
	...
	<div id="moveBtnGroup">
		...
		<div id="waitingListGroup">
			<form role="form" method="get">
				...
				<script type="text/javascript">
					$(function(){
						$('#searchBtn').click(function(){
							self.location = "documentWaitingList" + '${paging.makeQuery(1)}' + "&searchType=" + $('select option:selected').val() + "&keyword=" + encodeURIComponent($('#keyword').val());
						});
					});
				</script>
				<p>여기가 대기리스트입니다</p>
				...
				<colgroup>
					...
					<tr>
						<th id="waiting_th">순번</th>
						<th id="waiting_th">문서번호</th>
						<th id="waiting_th">기안 유형</th>
						<th id="waiting_th">기안 부서</th>
						<th id="waiting_th">기안자</th>
						<th id="waiting_th">제목</th>
						<th id="waiting_th">기안 일시</th>
					</tr>
					<%
						for(int i=0; i<waiting.size(); i++){
							ApprovalVO vo = waiting.get(i);
					%>
					<tr>
						<td><%=vo.getEidx() %></td>
		...
		<div id="paging">
			...
			%>
					<a href="/approval/documentWaitingList<%= paging.makeSearch(paging.getStartPage()-1) %>">&lt;</a>
			<%
				}
				for(int i=paging.getStartPage(); i<=paging.getEndPage(); i++){
			%>
					<a href="/approval/documentWaitingList<%= paging.makeSearch(i) %>"><%=i %></a>
			<%		
				}
				if(paging.isNext() && paging.getEndPage()>0){
			%>
					<a href="/approval/documentWaitingList<%= paging.makeSearch(paging.getEndPage()+1) %>">&gt;</a>
			<%
				}
			%>
		</div>
