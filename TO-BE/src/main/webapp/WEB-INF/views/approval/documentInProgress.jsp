...
<% 
	List<ApprovalVO> progress = (List<ApprovalVO>)request.getAttribute("progress");
...
<html>
	...
	<body>
		...
		<div id="progressListGroup">
			<form role="form" method="get">
				...
				<script type="text/javascript">
					$(function(){
						$('#searchBtn').click(function(){
							self.location = "documentInProgress" + '${paging.makeQuery(1)}' + "&searchType=" + $('select option:selected').val() + "&keyword=" + encodeURIComponent($('#keyword').val());
						});
					});
				</script>
				<p>여기가 진행중리스트입니다</p>
				...
				<colgroup>
					...
					<tr>
						<th id="progress_th">순번</th>
						<th id="progress_th">문서번호</th>
						<th id="progress_th">기안 유형</th>
						<th id="progress_th">기안 부서</th>
						<th id="progress_th">기안자</th>
						<th id="progress_th">제목</th>
						<th id="progress_th">기안 일시</th>
					</tr>
					<%
						for(int i=0; i<progress.size(); i++){
							ApprovalVO vo = progress.get(i);
					%>
					<tr>
						<td><%=vo.getEidx() %></td>
		...
		<div id="paging">
			...
			%>
					<a href="/approval/documentInProgress<%= paging.makeSearch(paging.getStartPage()-1) %>">&lt;</a>
			<%
				}
				for(int i=paging.getStartPage(); i<=paging.getEndPage(); i++){
			%>
					<a href="/approval/documentInProgress<%= paging.makeSearch(i) %>"><%=i %></a>
			<%		
				}
				if(paging.isNext() && paging.getEndPage()>0){
			%>
					<a href="/approval/documentInProgress<%= paging.makeSearch(paging.getEndPage()+1) %>">&gt;</a>
			<%
				}
			%>
		</div>
